...
public class KThread {
	...
	public void fork() {
		...
		System.out.println("Before thread "+ currentThread.getName() +" Start");
		tcb.start(new Runnable() {
			public void run() {
				runThread();
			}
		});
		System.out.println("After thread "+ currentThread.getName() +" Start");

		ready();

		Machine.interrupt().restore(intStatus);

		System.out.println("Restore interrupt");
	}
	...
	public static void finish() {
		...
		if(joinTable.containsKey(currentThread)){
			joinTable.get(currentThread).ready();
			joinTable.remove(currentThread);
		}
	...
	public static void yield() {
		...
		Lib.debug(dbgThread, "Yielding thread pair: " + currentThread.toString());

		Machine.interrupt().restore(intStatus);
	}

	public static void waitUntilYield() {
		Lib.debug(dbgThread, "Yielding thread: " + currentThread.toString());
	...
	public void join() {
		...
		Lib.assertTrue(!calledJoinThreads.contains(this));

		boolean intStatus = Machine.interrupt().disable();

		calledJoinThreads.add(this);
		...
		if(this.status==statusFinished){
			...
			return;
				...
				joinTable.put(this,currentThread);
				KThread callingThread=currentThread;
				KThread calledThread=this;
				System.out.println("Calling "+currentThread.getName());
				System.out.println("Called "+this.getName());
				System.out.println(Machine.interrupt().disabled());
				while(calledThread.status!=statusFinished){
					KThread.currentThread().sleep();
				}
//				while(calledThread.status!=statusFinished){
//					KThread.currentThread().yield();x
//				}
        }
		Machine.interrupt().restore(intStatus);
	}
	...
	System.out.println(child1.status);
	child1.join();
    ...
    private static void joinTest2(){
		System.out.println("Enter jointtest2");
		class Domino implements Runnable{
			private KThread kthread;
			public Domino(KThread kthread){
				this.kthread=kthread;
			}
			public void run(){
				kthread.join();
				System.out.println(KThread.currentThread().getName()+" "+"terminate");
			}
		}
		KThread previous = new KThread( new Runnable () {
			public void run() {
				System.out.println("I (heart) Nachos!");
			}
		});
		previous.setName("initialize").fork();
		for(int i=0;i<10;i++){
			KThread kthread=new KThread(new Domino(previous)).setName(""+i);
			kthread.fork();
			previous=kthread;
		}
		previous.join();
	...
	private void run() {
		...
		currentThread = this;
//		System.out.println("Before Context Switch");
		tcb.contextSwitch();
//		System.out.println("After Context Switch");
//		System.out.println("After context switch"+currentThread.getName());
...
//			}
	...
	private static Hashtable<KThread,KThread> joinTable=new Hashtable<>();

	private static HashSet<KThread> calledJoinThreads=new HashSet<>();
}
