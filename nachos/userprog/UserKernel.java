...
public class UserKernel extends ThreadedKernel {
	...
	public TranslationEntry[] getPages(int num){
		//Lib.assertTrue(num==1);
		System.out.println("Xingfei getPages." + pages.size());
...
//		if(pages.isEmpty() || num > pages.size()) return res;
	...
	public void clearPages(TranslationEntry[] pageTables){
		lock.acquire();
		for(TranslationEntry tmp:pageTables){
			if(tmp.valid){
				tmp.valid=false;
				pages.add(tmp);
			}
	...
	protected Lock lock; // for get/clear Pages

	private LinkedList<TranslationEntry> pages = new LinkedList <TranslationEntry>();
